...
while True:
    ...
    for vacancy in vacancy_list:
        ...
        payment = vacancy.find('div', {'class': 'vacancy-serp-item__sidebar'})

        if payment:
            min_payment = payment.getText()
        ...
        if payment:
            currency_payment = payment.getText()
        ...
        vacancy_id = re.findall('(\d+)\?', vacancy_link)

        vacancy_data['_id'] = int(vacancy_id[0])
...
for vacancydb in vacancydb.find( {'$or': [ {'min_payment': {'$gt' : sum}}, {'max_payment': {'$gt' : sum}}]}):
    print(vacancydb)
